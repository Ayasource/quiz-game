// Simplified quiz script - handles geography quiz and small index leaderboard preview
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function enableLeaderboardLinks() {
    ...
    // Prefer focusing the modal link when modal will be shown; otherwise focus the page link
  ...
  // small index leaderboard preview
  ...
  if (lbContainer) {
    try {
      ...
      if (!list.length) lbContainer.innerHTML = '<li class="text-muted">No scores yet</li>';
      else { list.sort((a,b) => (b.score - a.score) || (new Date(a.date) - new Date(b.date))); const top = list.slice(0,5); lbContainer.innerHTML = top.map(item => `<li>${escapeHtml(item.name||'Player')} — <strong>${escapeHtml(String(item.score))}</strong></li>`).join(''); }
